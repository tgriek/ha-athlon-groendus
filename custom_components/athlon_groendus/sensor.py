...
async def async_setup_entry(
    ...
    coordinator = AthlonGroendusCoordinator(
        ...
        chargepoint_id=entry.data[CONF_CHARGEPOINT_ID],
        update_interval_seconds=int(entry.options.get(CONF_UPDATE_INTERVAL, DEFAULT_UPDATE_INTERVAL_SECONDS)),
        max_pages=int(entry.options.get(CONF_MAX_PAGES, DEFAULT_MAX_PAGES)),
    )

    # First refresh so entities start with data
